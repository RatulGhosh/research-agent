...
def create_novelty_analyst(llm, tools, max_search_calls: int = 4):
    ...
    def novelty_analyst_node(state) -> dict:
        ...
        system_prompt = (
            ...
            "literature.\n\n"
            "Use your search tools to survey related work: search_literature for "
            "arXiv preprints, and web_search (when available) for very recent "
            "results, published venue versions, lab blog posts, and open-source "
            "projects. Issue several targeted queries with different phrasings "
            "and subtopics before concluding — do not rely on a single query. "
